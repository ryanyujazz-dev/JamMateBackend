# Voicing Module Core Logic V2 — Compact
Current version: `v2_2_73`.
## Current chain
```text
chord symbol / harmony material
→ content_planner / canonical_source / four_note_sources / source_balance
→ texture_plan VoicingTextureIntent / VoicingTextureState
→ disposition projection
→ scorer / selector / voice_leading
→ VoicingPlan + projection_map
```
## Module responsibilities
- `content_planner.py`: chord-valid source/content decisions.
- `color_permission.py`: chord-symbol-only / explicit-color / expansion gate.
- `source_balance.py`: source balance weights; No new subfolder.
- `texture_plan.py`: `VoicingTextureIntent`, `VoicingTextureState`, `derive_voicing_texture_intent`, `derive_voicing_texture_state`, `voicing_texture_state_engine_resolved_contract_v2_2_21`.
- `disposition/closed.py`: `place_compact_closed_seed_layout`, `strict_closed_register_variants`, `closed_projection_migrated`.
- `disposition/open.py`: `place_drop2_projection_from_closed_parent`, `place_drop3_projection_from_closed_parent`, `place_drop2_and_4_projection_from_closed_parent`, `place_named_open_projection_from_closed_parent`, `open_drop2_parent_closed_notes`, `open_drop2_from_parent_closed_projection`; named methods must be not from a direct source-order compact stack.
- `disposition/spread.py`: lower/upper grouped spread methods.
## Historical contract labels retained
`v2_2_2 — Closed Projection Migration / No Behavior Change`; `v2_2_3 — Legacy Disposition Cleanup Pass`; `v2_2_4 — Open Projection Skeleton / DROP2 4-note Only`; `v2_2_5 update — DROP2 Parent-Closed Projection Correction Pass`; `v2_2_7 update — OPEN Method Candidate Pool / DROP3-DROP2&4 Skeleton Pass`; `Progression / Phrase-level Voicing Method Lock`; `open_projection_method_pool`; `DROP3`; `DROP2_AND_4`; `disposition/facade.py is now only a placement facade`; `src/jammate_engine/core/voicing/disposition/open.py`; `src/jammate_engine/core/voicing/disposition/spread.py`; `src/jammate_engine/core/voicing/disposition/placement_utils.py`.
## SPREAD notes boundary
SPREAD notes must be implemented in core voicing, not in style generators and not in expression. The projection contract is lower functional group + upper source/projection group.
Reusable inputs: existing 3-note/4-note source selection, orientation families, color permission, chord-quality resolver, `four_note_sources.py`, and DROP2/DROP3 projection resources. Forbidden shortcut: taking a final placed closed/open voicing and appending a lower root as the SPREAD result. v2_2_38 exposes this as `spread_recipe_reuse_audit()` plus `spread_recipe_contract_skeleton()` in `core/voicing/disposition/spread.py`, with `SPREAD_RECIPE_CONTRACT_VERSION`, `SpreadRecipeContract`, `LowerGroupRecipeContract`, `UpperSourceRef`, and `SpreadReuseAuditItem`.
Future selector/scorer work should calculate lower-group motion, upper-group motion, top-voice motion, group-gap stability, and span/register penalties separately. This should allow natural ii-V-I lower movement such as `root+7 -> root+3 -> root+7` or `root+3 -> root+7 -> root+3`.
...
## v2_2_28 voicing texture runtime boundary
Family continuity is now a core voicing responsibility, not a style pattern trick. Styles may enable texture-state runtime filtering through metadata; core candidate generation consumes the resolved state and filters CLOSED/OPEN/SPREAD families before low-level projection. MethodLock still handles progression-level method continuity.
...
## v2_2_38 texture-scope runtime boundary
`PatternEvent.metadata` is the bridge from chord-region form context to voicing texture state. `HarmonicRealizer` may attach event-scoped texture metadata to `VoicingPolicy`, but candidate generation and disposition projection remain core voicing responsibilities.
## v2_2_38 lower group inventory boundary
Lower Group Recipe Inventory is core voicing/disposition logic, not a style generator rule. It resolves lower/foundation roles through Harmony material: root, chord-quality third, chord-quality fifth, and explicit seventh/sixth when available. It does not silently add a seventh to plain triads. The public helpers are `LOWER_GROUP_INVENTORY_VERSION`, `LowerGroupRecipeId`, `LowerGroupRecipeInventoryItem`, `instantiate_lower_group_recipe`, `place_lower_group_recipe`, and `lower_group_inventory_debug`. Recipes remain notes-only, within one octave, and free of expression/pedal behavior.
## v2_2_40 upper source adapter boundary
`Upper Source Adapter` is core voicing/disposition logic, not a style generator. It lives in the existing `spread.py` owner and exposes `UPPER_SOURCE_ADAPTER_VERSION`, `SpreadUpperSourceOption`, `SpreadUpperSourceAdapterResult`, `adapt_spread_upper_source`, and `spread_upper_source_adapter_debug`. The adapter calls `content_planner` for existing source/orientation/color-gated degree options, preserves metadata such as canonical inversion/source order and explicit chart-color gates, and references DROP2/DROP3 as reusable projection resources. Its output is source_oriented_not_placed, notes-only, and runtime-disabled; it never treats a final placed closed/open voicing as the SPREAD upper block.
...
## v2_2_58 SPREAD 2+3 lower foundation fix
- `spread_2plus3_contract` remains explicit listening-isolation only; default Jazz Ballad runtime is unchanged.
- SPREAD lower 2-note rooted mode uses `root+3`, `root+5`, and `root+7` as equal-weight recipe choices. Rootless `3+7` is a separate explicit mode and must not be randomly mixed into rooted lower-foundation phrases.
- SPREAD lower 2-note register is fixed to A0-E2, and the group top note must be at least G1.
- SPREAD upper 3-note shell+1/root options are excluded from the upper pool.
- Dual-demo workflow remains required: unexpanded and expansion-enabled standard-tune demos; expansion-enabled target remains about 60% actual color while preserving v2_2_54 source integrity.
...
- v2_2_88 — Minor V→i / Turnaround Altered Coverage Audit: altered-dominant scope now records both final `functional_scope` and `inferred_functional_scope`, so explicit altered symbols preserve chart fidelity while audit still sees whether the underlying motion is resolving V7, secondary, or turnaround-related. Key-aware examples include `Bm7b5 → E7 → Am` in A minor vs C major.
