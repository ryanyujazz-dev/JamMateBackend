# Disposition Projection Architecture V2 — Compact
Current version: `v2_2_73`.
## Taxonomy
- `DispositionFamily`: CLOSED / OPEN / SPREAD.
- `OpenProjectionMethod`: GENERIC_OPEN / DROP2 / DROP3 / DROP2_AND_4.
- `SpreadProjectionMethod`: lower/upper grouped and foundation projection variants.
- `legacy Disposition` is normalized through `LEGACY_DISPOSITION_PROJECTION_MAP`.
- resolver migration is not part of v2_2_x.
## Entry
`project_source_to_disposition()` returns `DispositionProjectionResult`. It lives with `src/jammate_engine/core/voicing/disposition/models.py` and `src/jammate_engine/core/voicing/disposition/projection.py`. The projection entry supports `legacy_placement_callback`, `closed_parent_placement_callback`, `legacy_projection_callback_used`, and `legacy_projection_cleanup_required`.
## Migration contracts
- `v2_2_2 — Closed Projection Migration / No Behavior Change`: `src/jammate_engine/core/voicing/disposition/closed.py`, `place_compact_closed_seed_layout`, `strict_closed_register_variants`, `closed_projection_migrated`.
...
- `v2_2_5 update — DROP2 Parent-Closed Projection Correction Pass`: `place_drop2_projection_from_closed_parent`, `open_drop2_parent_closed_notes`, `not from a direct source-order compact stack`.
- `v2_2_7 update — OPEN Method Candidate Pool / DROP3-DROP2&4 Skeleton Pass`: `Progression / Phrase-level Voicing Method Lock`, `open_projection_method_pool`, `DROP3`, `DROP2_AND_4`.
## Texture relationship
VoicingTextureIntent / VoicingTextureState describe phrase/section `family` language above this low-level Disposition Projection. `method lock` controls progression method continuity. Capability Reuse Before New Construction applies.
## SPREAD projection planning contract
SPREAD is not an OPEN method. OPEN/drop-family projects a full voicing from a closed parent. SPREAD projects two functional layers: lower foundation group plus upper source/projection group.
Planned recipes:
- `1+3`, `2+2`, `1+4`, `2+3`, `2+4`, `3+3`, later `3+4`.
- 1-note lower uses root only.
- 2-note lower uses `root+7`, `root+3`, `root+5`, `3+7`, within one octave.
- 3-note lower uses `root+5+upper_root`, `root+3+7`, `root+7+upper3`, `root+5+upper3`, within one octave.
`1+4` should use lower root plus an upper 4-note DROP2/DROP3-derived block. Reuse source/orientation/drop resources, then let SPREAD own group placement, gap guard, span guard, and group-wise continuity.
...
## SPREAD recipe reuse contract
`src/jammate_engine/core/voicing/disposition/spread.py` now owns the v2_2_38 notes/projection skeleton: `SPREAD_RECIPE_CONTRACT_VERSION`, `SpreadGrouping`, `LowerGroupRecipeContract`, `UpperSourceRef`, `SpreadRecipeContract`, `SpreadReuseAuditItem`, `spread_recipe_reuse_audit()`, and `spread_recipe_contract_skeleton()`. Reuse level is source/orientation/projection-resource only; final placed closed/open `VoicingCandidate` results are not reusable as SPREAD placements. `1+4` references upper DROP2/DROP3-derived projection resources but SPREAD owns lower/upper register, gap, span, and group-wise voice-leading.
## v2_2_38 lower group recipe inventory
`core/voicing/disposition/spread.py` now owns the notes-only Lower Group Recipe Inventory through `LOWER_GROUP_INVENTORY_VERSION`, `LowerGroupRecipeId`, `LowerGroupRecipeInventoryItem`, `instantiate_lower_group_recipe`, `place_lower_group_recipe`, and `lower_group_inventory_debug`. Lower groups are chord-quality-aware and use Harmony-owned degree resolution for third/fifth/seventh. The inventory covers 1-note `root`; 2-note `root+7 / root+3 / root+5 / 3+7`; and 3-note `root+5+upper_root / root+3+7 / root+7+upper3 / root+5+upper3`. Placement enforces register legality and span guards while emitting lower/foundation group metadata; 3-note lower spread groups may span up to 16 semitones. This is not expression, pedal, or Ballad runtime logic.
## v2_2_40 upper source adapter
The SPREAD projection foundation now includes `Upper Source Adapter` helpers in `core/voicing/disposition/spread.py`: `UPPER_SOURCE_ADAPTER_VERSION`, `SpreadUpperSourceOption`, `SpreadUpperSourceAdapterResult`, `spread_upper_source_refs`, `adapt_spread_upper_source`, and `spread_upper_source_adapter_debug`. The adapter reuses `content_planner`, existing 3-note/4-note orientation metadata, shared color permission, and DROP2/DROP3 projection-resource refs. It deliberately produces source_oriented_not_placed material rather than a final candidate; final placed closed/open results remain non-reusable.
