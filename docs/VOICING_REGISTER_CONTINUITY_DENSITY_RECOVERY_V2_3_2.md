# v2_3_9 — Voicing Register Continuity / Density Recovery
...
v2_3_9 的目标不是继续扩张新 voicing source，而是恢复之前 Ballad SPREAD / functional grouping 的厚度与自然音区连续性。
...
在 `core/voicing/selection/scorer.py` 中加入 `REGISTER_CONTINUITY_RECOVERY_VERSION = v2_3_9`，并把 register continuity 作为 candidate scoring 的一个独立分量：
...
v2_3_9 继续使用 `tools/demo_audit_pipeline.py` 作为统一入口，并增强 Demo Matrix listening regression thresholds：
...
Misty Ballad v2_3_9 demo 的目标状态：
...
下一步不应继续扩大 Demo Matrix，而应先做 v2_3_9：
