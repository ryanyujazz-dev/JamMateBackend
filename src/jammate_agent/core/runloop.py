...
RUNLOOP_CONTRACT_VERSION = "v2_4_13"
...
class BoundedRunLoopPolicy:
    ...
    v2_4_13 keeps LLM calls preview/config-guarded and does not execute autonomous tools. The policy
...
class BoundedAgentRunLoop:
    ...
    def preview(self, context_packet: ContextPacket) -> RunLoopPreviewResult:
        ...
        if llm_required and llm_provider_configured:
            ...
            reason = "LLM provider config is present, but v2_4_13 only exposes the provider boundary, request envelope, and tool invocation preview contract; no LLM call is executed."
        elif llm_required:
