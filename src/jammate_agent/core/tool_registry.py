...
_TOOL_DESCRIPTORS: tuple[AgentToolDescriptor, ...] = (
    ...
    AgentToolDescriptor(
        name="agent_routine_config_prepare",
        title="Agent Routine Config Prepare",
        description="Prepare an editable HarmonyOS RoutineConfig candidate from user intent, a practice plan, or a practice block without starting playback.",
        category="routine_setup",
        task_types=(),
        direct_client_callable=True,
        deterministic_workflow="RoutineConfigPreparer.prepare_candidate",
        route="POST /agent/actions/routine-config/prepare",
        input_contract={"route": "POST /agent/actions/routine-config/prepare", "schema": "RoutineConfigPrepareRequest"},
        output_contract={"schema": "RoutineConfigPrepareActionPayload", "playback_started": False, "asset_format": None},
