...
MILESTONE_ID = "v2_6_77"
MILESTONE_LABEL = "v2_6_77 — Medium Swing Existing Voicing Capability Usage Policy"
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": (
            "Medium Swing can request already-existing grouped SPREAD 5/6-note capability from style/event policy when explicitly enabled. "
            "Ordinary runtime comping remains OPEN/DROP 4-note by default. The task does not modify core voicing source, projection, "
            "selector, expression, pattern, API, Agent, or HarmonyOS behavior."
        ),
        "static_audit": static_audit,
        "outputs": outputs,
        "aggregate": _aggregate(outputs),
        "acceptance": _acceptance(static_audit, outputs),
        "recommended_next_task": "medium_swing_existing_voicing_capability_listening_refinement_if_needed_else_full_band_checkpoint",
    }
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_existing_voicing_capability_usage_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_existing_voicing_capability_usage_report.md"
    ...
    return {
        ...
        "metadata": {
            "medium_swing_existing_voicing_capability_usage_policy": {
                ...
                "available": True,
                "enabled": True,
                "activation": "explicit_v2_6_77_audit_demo",
...
def _generate_and_audit(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    slug = str(spec["slug"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{slug}_medium_swing_existing_voicing_capability_usage_demo.mid"
...
def _acceptance(static_audit: dict[str, Any], outputs: list[dict[str, Any]]) -> dict[str, Any]:
    aggregate = _aggregate(outputs)
    checks = {
        "policy_version_declared": static_audit.get("arrangement_policy_version") == MILESTONE_ID and static_audit.get("voicing_policy_version") == MILESTONE_ID,
