...
MILESTONE_LABEL = "v2_6_103 — Engine Bossa Nova OPEN Voicing + Retired 4-Note Grouping Metadata"
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": (
            "Make Bossa piano voicing OPEN-main while keeping normal 4-to-5-note voicing density. "
            "Retire the old ordinary 4-note 1+3 / 2+2 functional-grouping metadata from core taxonomy so grouped voicing reports refer only to genuine SPREAD lower/upper contracts. "
            "No Bossa parallel selector, no pattern vocabulary change, no expression numeric change, no API/Agent/HarmonyOS change."
        ),
        "static_audit": static_audit,
        "runtime_audits": runtime_audits,
        "acceptance": acceptance,
        "recommended_next_task": "v2_6_104_engine_bossa_nova_kick_bass_lock_and_low_frequency_shadow_refinement",
    }
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_engine_bossa_nova_open_voicing_taxonomy_cleanup_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_engine_bossa_nova_open_voicing_taxonomy_cleanup_report.md"
...
def _generate_runtime_audit(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    seed = int(spec["seed"])
    slug = str(spec["slug"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{slug}_bossa_nova_open_voicing_taxonomy_cleanup_demo.mid"
...
def _acceptance(static: dict[str, Any], runtimes: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    checks = {
        "style_and_arrangement_registered": static.get("style_registered") is True and static.get("arrangement_policy_version") == MILESTONE_ID,
