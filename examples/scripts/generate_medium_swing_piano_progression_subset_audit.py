...
MILESTONE_ID = "v2_6_65"
MILESTONE_LABEL = "v2_6_65 — Medium Swing Progression-Specific Candidate Subset Policy"
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": "V1-derived progression-specific Medium Swing piano candidate priority translated into V2 ChordRegion-first preferred-subset reweighting. No bar-first templates, voicing selection, final expression values, gesture, API, Agent, or HarmonyOS behavior is introduced.",
    ...
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_piano_progression_subset_audit_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_piano_progression_subset_audit_report.md"
...
def _generate_and_audit(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    slug = str(spec["slug"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{slug}_medium_swing_piano_progression_subset_demo.mid"
    ...
    for output in summary.get("outputs", []):
        lines.extend(
            [
                ...
                f"- Hold-until-next-touch events: `{output['hold_until_next_touch_applied_events']}`",
