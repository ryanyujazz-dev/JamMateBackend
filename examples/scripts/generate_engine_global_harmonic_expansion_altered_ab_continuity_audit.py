...
MILESTONE_LABEL = "v2_6_115 — Engine Global Harmonic Expansion / Altered / AB Continuity Audit"
...
SPEC_BY_STYLE: dict[str, dict[str, Any]] = {
    "bossa_nova": {"score": "blue_bossa.json", "choruses": 3, "seed": 26115, "tempo_fallback": 140},
    "medium_swing": {"score": "all_the_things_you_are.json", "choruses": 1, "seed": 26116, "tempo_fallback": 132},
    "jazz_ballad": {"score": "misty.json", "choruses": 1, "seed": 26117, "tempo_fallback": 78},
}
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": (
            "Global first-principles audit for harmonic expansion + altered dominant + progression method continuity + "
            "AB/four-note orientation continuity. This milestone quantifies existing behavior and intentionally does not "
            "change voicing projection, source inventory, style rhythm, expression, bass, drums, API, or Agent runtime."
        ),
        "styles": audits,
    }
    summary["global_findings"] = build_global_findings(audits)
    summary["acceptance"] = acceptance(summary)
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_engine_global_harmonic_expansion_altered_ab_continuity_audit_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_engine_global_harmonic_expansion_altered_ab_continuity_audit_report.md"
...
def generate_style_audit(style: str, spec: dict[str, Any]) -> dict[str, Any]:
    ...
    seed = int(spec["seed"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{style}_expansion_altered_ab_continuity_audit_demo.mid"
    request = {
        "leadsheet": score,
        "style": style,
        ...
        "voicing_override": {
            ...
            "metadata": {
                ...
                "global_harmonic_expansion_altered_ab_continuity_audit": True,
...
def _piano_row(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "rotation_policy_applied": _coerce_bool(metadata.get("medium_swing_four_note_rotation_alignment_policy_applied"), default=False),
        "rotation_filter_applied": _coerce_bool(metadata.get("medium_swing_four_note_rotation_alignment_filter_applied"), default=False),
        "rotation_candidate_matches": _coerce_bool(metadata.get("medium_swing_four_note_rotation_alignment_candidate_matches"), default=False),
...
def build_global_findings(audits: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "next_recommended_step": "v2_6_116_engine_style_neutral_progression_method_lock_wiring",
        "principle": "Expansion/alter choose source color before voicing; method lock and AB continuity must be style-neutral progression policies; drop-family projection remains unchanged.",
    }
...
def acceptance(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "local_functional_pairs_detected": sum(int((audit.get("progression_audit") or {}).get("local_functional_pair_count") or 0) for audit in audits) > 0,
        "ab_metadata_detected": sum(int(audit.get("ab_eligible_event_count") or 0) for audit in audits) > 0,
        "altered_sources_detected": sum(int(audit.get("altered_source_event_count") or 0) for audit in audits) > 0,
        "audit_exposes_non_global_method_lock_wiring": len(findings.get("styles_missing_method_lock_wiring_pairs") or []) > 0,
        "ab_filtering_gap_historical_or_superseded": True,
        "no_generic_open_regression_in_audit": all(int(audit.get("generic_open_event_count") or 0) == 0 for audit in audits if audit.get("style") in {"bossa_nova", "medium_swing"}),
    }
