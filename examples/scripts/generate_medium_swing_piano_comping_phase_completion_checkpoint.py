...
MILESTONE_ID = "v2_6_76"
MILESTONE_LABEL = "v2_6_76 — Medium Swing Piano Comping Phase Completion Checkpoint"
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": (
            "Behavior-preserving Medium Swing piano comping phase-completion checkpoint. "
            "v2_6_76 summarizes the v2_6_56 through v2_6_74 comping line after the fill-frequency audit showed "
            "very low intrusion, confirming ChordRegion-first pattern routing, history continuity, expression-hint handoff, "
            "ending stability, optional-fill safety, and pattern/expression/voicing separation on standard-tune 3-chorus demos. "
            "No candidate weight, vocabulary, selector, voicing, expression numeric, API, Agent, or HarmonyOS behavior is changed."
        ),
    ...
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_piano_comping_phase_completion_checkpoint_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_piano_comping_phase_completion_checkpoint_report.md"
...
def build_static_audit() -> dict[str, Any]:
    ...
    return {
        ...
        "checkpoint_policy_enabled": bool(policy.get("medium_swing_piano_comping_phase_completion_checkpoint")),
        "checkpoint_policy_version": str(policy.get("medium_swing_piano_comping_phase_completion_checkpoint_version")),
        "checkpoint_policy_contract": str(policy.get("medium_swing_piano_comping_phase_completion_checkpoint_contract")),
        "fill_frequency_checkpoint_policy_version": str(policy.get("piano_comping_standard_tune_fill_frequency_checkpoint_version")),
        "fill_frequency_checkpoint_policy_enabled": bool(policy.get("piano_comping_standard_tune_fill_frequency_checkpoint")),
...
def _generate_and_audit(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    slug = str(spec["slug"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{slug}_medium_swing_piano_comping_phase_completion_checkpoint_demo.mid"
        ...
        rows.append(
            {
                ...
                "phase_completion_checkpoint_version": metadata.get("medium_swing_piano_comping_phase_completion_checkpoint_version"),
                "phase_completion_checkpoint_applied": bool(metadata.get("medium_swing_piano_comping_phase_completion_checkpoint_applied")),
                "phase_completion_checkpoint_scope": metadata.get("medium_swing_piano_comping_phase_completion_checkpoint_scope"),
...
def _pattern_summary(pattern_rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "fill_frequency_checkpoint_metadata_events": sum(1 for row in pattern_rows if row.get("fill_frequency_checkpoint_version") == "v2_6_74"),
        "phase_completion_checkpoint_metadata_events": sum(1 for row in pattern_rows if row.get("phase_completion_checkpoint_version") == MILESTONE_ID),
...
def _acceptance(static: Mapping[str, Any], outputs: list[Mapping[str, Any]]) -> dict[str, Any]:
    checks: dict[str, bool] = {
        ...
        "static_fill_frequency_policy_version": static.get("fill_frequency_checkpoint_policy_version") == "v2_6_74",
        ...
        checks.update(
            {
                ...
                "phase_completion_checkpoint_metadata_present": all(output.get("pattern_summary", {}).get("phase_completion_checkpoint_metadata_events", 0) > 0 for output in outputs),
...
def _render_report(summary: Mapping[str, Any]) -> str:
    ...
    lines.append(f"- Phase checkpoint policy: `{static['checkpoint_policy_version']}` enabled={static['checkpoint_policy_enabled']}")
    lines.append(f"- Prior fill-frequency checkpoint: `{static['fill_frequency_checkpoint_policy_version']}` enabled={static['fill_frequency_checkpoint_policy_enabled']}")
    ...
    lines.append("## Aggregate Phase Safety Summary")
