...
MILESTONE_ID = "v2_6_60"
MILESTONE_LABEL = "v2_6_60 — Medium Swing Harmonic-Function-Aware Piano Comping Policy"
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": "Audit the v2_6_60 Medium Swing harmonic-function-aware piano comping policy. The policy reweights the existing ChordRegion-length candidate pool by functional motion before the v2_6_59 history scorer; it does not introduce a bar-first/two-chord-bar route, parallel selector, voicing rule, expression parameter, or gesture.",
        "outputs": outputs,
        "acceptance": _acceptance(outputs),
    }
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_piano_harmonic_function_audit_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_piano_harmonic_function_audit_report.md"
...
def _generate_and_audit(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    slug = str(spec["slug"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{slug}_medium_swing_piano_harmonic_function_demo.mid"
    ...
    harmonic_label_counts = Counter(str(row.get("harmonic_label") or "missing") for row in region_rows)
    harmonic_reason_counts = Counter(reason for row in rows for reason in row.get("harmonic_reasons") or [])
    return {
        "ok": bool(result.ok),
        ...
        "harmonic_label_counts": dict(harmonic_label_counts),
        "harmonic_reason_counts": dict(harmonic_reason_counts),
        ...
        rows.append(
            {
                ...
                "harmonic_version": metadata.get("harmonic_function_comping_policy_version"),
                "harmonic_applied": bool(metadata.get("harmonic_function_comping_policy_applied", False)),
                "harmonic_label": metadata.get("harmonic_function_context_label"),
                "harmonic_multiplier": float(metadata.get("harmonic_function_multiplier") or 1.0),
                "harmonic_reasons": list(metadata.get("harmonic_function_reasons") or []),
                "harmonic_previous_to_current_type": metadata.get("harmonic_function_previous_to_current_type"),
                "harmonic_current_to_next_type": metadata.get("harmonic_function_current_to_next_type"),
                "harmonic_window_type": metadata.get("harmonic_function_window_type"),
...
def _metadata_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    history_versions = Counter(str(row.get("history_version") or "missing") for row in rows)
    history_reasons = Counter(reason for row in rows for reason in row.get("history_reasons") or [])
    history_penalized = [row for row in rows if float(row.get("history_multiplier") or 1.0) < 1.0]
    history_bonus = [row for row in rows if float(row.get("history_multiplier") or 1.0) > 1.0]
    harmonic_versions = Counter(str(row.get("harmonic_version") or "missing") for row in rows)
    harmonic_reasons = Counter(reason for row in rows for reason in row.get("harmonic_reasons") or [])
    harmonic_penalized = [row for row in rows if float(row.get("harmonic_multiplier") or 1.0) < 1.0]
    harmonic_bonus = [row for row in rows if float(row.get("harmonic_multiplier") or 1.0) > 1.0]
    return {
        "harmonic_version_counts": dict(harmonic_versions),
        "harmonic_applied_events": sum(1 for row in rows if row.get("harmonic_applied")),
        "harmonic_penalized_events": len(harmonic_penalized),
        "harmonic_bonus_events": len(harmonic_bonus),
        "harmonic_reason_counts": dict(harmonic_reasons),
        "history_version_counts": dict(history_versions),
        "history_applied_events": sum(1 for row in rows if row.get("history_applied")),
        "history_penalized_events": len(history_penalized),
        "history_bonus_events": len(history_bonus),
        "history_reason_counts": dict(history_reasons),
    }
    ...
    for output in outputs:
        ...
        checks.extend(
            [
                ...
                {"name": f"{output['slug']}: harmonic function metadata applied", "passed": metadata.get("harmonic_applied_events", 0) == output.get("piano_events", -1)},
