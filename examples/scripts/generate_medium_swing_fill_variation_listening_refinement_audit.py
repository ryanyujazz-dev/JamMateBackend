...
MILESTONE_ID = "v2_6_72"
MILESTONE_LABEL = "v2_6_72 — Medium Swing Fill/Variation Listening Refinement After User Review"
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": (
            "Medium Swing piano fill/variation listening refinement after user review. v2_6_72 keeps the v2_6_71 "
            "three-candidate optional vocabulary unchanged and only refines context/history multipliers: variation remains "
            "low-intrusion, transition fill prefers phrase/section/turnaround contexts, and busy fill stays heavily guarded. "
            "No fill selector, bar-first phrase route, voicing logic, or final expression values are introduced."
        ),
    ...
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_fill_variation_listening_refinement_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_fill_variation_listening_refinement_report.md"
...
def _generate_and_audit(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    slug = str(spec["slug"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{slug}_medium_swing_fill_variation_listening_refinement_demo.mid"
...
def _pattern_summary(pattern_rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "optional_refinement_metadata_events": sum(1 for row in pattern_rows if row.get("optional_fill_variation_listening_refinement_policy_version") == MILESTONE_ID),
...
def _acceptance(static: Mapping[str, Any], outputs: list[Mapping[str, Any]]) -> dict[str, Any]:
    checks: dict[str, bool] = {
        ...
        "static_refinement_policy_version": static.get("optional_refinement_policy_version") == MILESTONE_ID,
