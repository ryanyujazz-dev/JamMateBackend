...
MILESTONE_LABEL = "v2_6_110 — Engine Bossa Nova Drop-Family Closed-Parent Projection Fix"
...
def main() -> None:
    ...
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_engine_bossa_nova_drop_family_closed_parent_projection_fix_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_engine_bossa_nova_drop_family_closed_parent_projection_fix_report.md"
...
def build_static_audit() -> dict[str, Any]:
    ...
    return {
        ...
        "fix_location": "core/voicing/selection/candidate_generator.py::_project_closed_parent_candidates_for_named_open_projection",
        "fix_contract": "named OPEN methods reuse existing compact CLOSED parent construction before DROP projection; fallback to the previous path is reserved for exotic unrepresentable sources",
    }
...
def _generate_runtime_audit(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    seed = int(spec["seed"])
    slug = str(spec["slug"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{slug}_bossa_nova_drop_family_closed_parent_projection_fix_demo.mid"
