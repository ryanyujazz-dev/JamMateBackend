...
MILESTONE_ID = "v2_6_73"
MILESTONE_LABEL = "v2_6_73 — Medium Swing Phrase-End Fill Context Precision"
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": (
            "Medium Swing phrase-end fill context precision. v2_6_73 keeps the v2_6_71/v2_6_72 optional "
            "fill vocabulary and listening-refinement layers unchanged, then narrows the existing transition-fill bonus "
            "toward explicit phrase ends, section tails, and 4/8-bar phrase tails while downweighting harmonic-transition-only regions. "
            "No phrase engine, fill selector, new vocabulary, voicing logic, or final expression values are introduced."
        ),
    ...
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_phrase_end_fill_context_precision_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_medium_swing_phrase_end_fill_context_precision_report.md"
...
def build_static_audit() -> dict[str, Any]:
    ...
    return {
        ...
        "optional_refinement_policy_enabled": bool(arrangement.get("piano_comping_optional_fill_variation_listening_refinement_policy")),
        "optional_refinement_policy_version": str(arrangement.get("piano_comping_optional_fill_variation_listening_refinement_policy_version")),
        "optional_policy_contract": str(arrangement.get("piano_comping_optional_fill_variation_vocabulary_contract")),
        "optional_refinement_policy_contract": str(arrangement.get("piano_comping_optional_fill_variation_listening_refinement_contract")),
        "phrase_end_precision_policy_enabled": bool(arrangement.get("piano_comping_phrase_end_fill_context_precision_policy")),
        "phrase_end_precision_policy_version": str(arrangement.get("piano_comping_phrase_end_fill_context_precision_policy_version")),
        "phrase_end_precision_policy_contract": str(arrangement.get("piano_comping_phrase_end_fill_context_precision_contract")),
...
def _generate_and_audit(spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    slug = str(spec["slug"])
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_{slug}_medium_swing_phrase_end_fill_context_precision_demo.mid"
        ...
        rows.append(
            {
                ...
                "optional_fill_variation_transition_context": bool(metadata.get("optional_fill_variation_transition_context")),
                "phrase_end_precision_policy_version": metadata.get("phrase_end_fill_context_precision_policy_version"),
                "phrase_end_precision_policy_applied": bool(metadata.get("phrase_end_fill_context_precision_policy_applied")),
...
def _pattern_summary(pattern_rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "phrase_end_precision_metadata_events": sum(1 for row in pattern_rows if row.get("phrase_end_precision_policy_version") == MILESTONE_ID),
...
def _acceptance(static: Mapping[str, Any], outputs: list[Mapping[str, Any]]) -> dict[str, Any]:
    checks: dict[str, bool] = {
        ...
        "static_policy_version": static.get("optional_policy_version") == "v2_6_71",
        "static_refinement_policy_enabled": static.get("optional_refinement_policy_enabled") is True,
        "static_refinement_policy_version": static.get("optional_refinement_policy_version") == "v2_6_72",
        "static_phrase_end_precision_policy_enabled": static.get("phrase_end_precision_policy_enabled") is True,
        "static_phrase_end_precision_policy_version": static.get("phrase_end_precision_policy_version") == MILESTONE_ID,
        ...
        checks.update(
            {
                ...
                "optional_refinement_metadata_present": all(output.get("pattern_summary", {}).get("optional_refinement_metadata_events", 0) > 0 for output in outputs),
...
def _render_report(summary: Mapping[str, Any]) -> str:
    ...
    lines.append(f"- Optional vocabulary policy: `{static['optional_policy_version']}` enabled={static['optional_policy_enabled']}")
    lines.append(f"- Optional refinement policy: `{static['optional_refinement_policy_version']}` enabled={static['optional_refinement_policy_enabled']}")
    lines.append(f"- Phrase-end precision policy: `{static['phrase_end_precision_policy_version']}` enabled={static['phrase_end_precision_policy_enabled']}")
