...
NO_EXPANSION_OUTPUT = DEMOS_DIR / "v2_2_89_misty_jazz_ballad_upper_structure_no_expansion_no_altered_demo.mid"
NO_EXPANSION_AUDIT_OUTPUT = DEMOS_DIR / "v2_2_89_misty_jazz_ballad_upper_structure_no_expansion_no_altered_audit_summary.json"
EXPANSION_OUTPUT = DEMOS_DIR / "v2_2_89_misty_jazz_ballad_upper_structure_expansion_only_demo.mid"
EXPANSION_AUDIT_OUTPUT = DEMOS_DIR / "v2_2_89_misty_jazz_ballad_upper_structure_expansion_only_audit_summary.json"
ALTERED_OUTPUT = DEMOS_DIR / "v2_2_89_misty_jazz_ballad_upper_structure_expansion_plus_altered_dominant_demo.mid"
ALTERED_AUDIT_OUTPUT = DEMOS_DIR / "v2_2_89_misty_jazz_ballad_upper_structure_expansion_plus_altered_dominant_audit_summary.json"
...
def _mix_audit(debug: dict[str, Any], *, expanded_demo: bool, altered_demo: bool) -> dict[str, Any]:
    ...
    upper_structure_register_scores: list[float] = []
    low_register_single_note_guard_events = 0
    low_register_single_note_guard_violations = 0
    ...
    for event in events:
        ...
        score_details = dict(dict(metadata.get("score_breakdown") or {}).get("details") or {})
        us_score = score_details.get("upper_structure_register_score")
        if us_score is not None:
            try:
                upper_structure_register_scores.append(float(us_score))
            except (TypeError, ValueError):
                pass
        register_guard = dict(voicing.get("register_guard") or metadata.get("register_guard") or {})
        low_single_ok = register_guard.get("low_register_single_note_ok", score_details.get("low_register_single_note_ok"))
        if low_single_ok is not None:
            low_register_single_note_guard_events += 1
            if low_single_ok is False:
                low_register_single_note_guard_violations += 1
    ...
    return {
        ...
        "scope": "Ballad SPREAD Upper Structure / Voicing Guard Listening Refinement with altered-dominant intensity; default runtime remains unchanged.",
        ...
        "upper_structure_register_score_avg": _avg(upper_structure_register_scores),
        "upper_structure_register_score_min": min(upper_structure_register_scores) if upper_structure_register_scores else None,
        "upper_structure_register_score_max": max(upper_structure_register_scores) if upper_structure_register_scores else None,
        "low_register_single_note_guard_events": int(low_register_single_note_guard_events),
        "low_register_single_note_guard_violations": int(low_register_single_note_guard_violations),
