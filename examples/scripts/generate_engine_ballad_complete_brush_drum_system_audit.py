...
MILESTONE_ID = "v2_6_129"
MILESTONE_LABEL = "v2_6_129 — Engine Ballad Complete Brush Drum System"
...
def main() -> None:
    ...
    summary = {
        ...
        "scope": (
            "Promote Jazz Ballad drums from sparse audible markers to a complete brush drum system: region-local circular brush texture, "
            "soft 2/4 anchors, feather kick, phrase breath, and final release. The change remains in Jazz Ballad percussion vocabulary "
            "plus shared percussion voice mapping and does not touch piano, bass, voicing, API, Agent, or HarmonyOS."
        ),
        "static_audit": static,
        "runtime_audit": runtime,
        "acceptance": acceptance,
        "recommended_next_task": "v2_6_130_engine_ballad_complete_brush_drum_listening_calibration",
    }
    summary_path = DEMOS_DIR / f"{MILESTONE_ID}_engine_ballad_complete_brush_drum_system_summary.json"
    report_path = DEMOS_DIR / f"{MILESTONE_ID}_engine_ballad_complete_brush_drum_system_report.md"
...
def _generate_misty_runtime_audit() -> dict[str, Any]:
    ...
    midi_path = DEMOS_DIR / f"{MILESTONE_ID}_misty_jazz_ballad_complete_brush_drum_system_demo.mid"
...
def _acceptance(static: dict[str, Any], runtime: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "ordinary_region_has_full_brush_kit": ordinary.get("candidate_count") == 1 and int((ordinary.get("event_drum_counts") or {}).get("brush_swirl", 0)) >= 4 and int((ordinary.get("event_drum_counts") or {}).get("brush_tap", 0)) >= 2 and int((ordinary.get("event_drum_counts") or {}).get("kick", 0)) >= 1,
        ...
        "misty_has_complete_drum_layer": runtime.get("ok") is True and 250 <= int(runtime.get("drum_note_events") or 0) <= 800,
