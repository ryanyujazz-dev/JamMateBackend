...
def test_context_and_runloop_surface_tool_invocation_preview_policy() -> None:
    ...
    assert summary["tool_invocation_preview_version"] == "v2_4_13"
...
def test_capability_and_runtime_specs_include_invocation_preview_boundary() -> None:
    ...
    assert runtime_spec["tool_invocation_preview_boundary"]["version"] == "v2_4_13"
    assert runtime_spec["routes"]["tool_invocation_preview"] == "POST /agent/tools/invocation/preview"
    assert "No runloop-driven tool execution in v2_4_13; tools are descriptor-only and invocation preview is validation-only." in runtime_spec["non_goals"]
