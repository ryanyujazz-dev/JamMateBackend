...
def test_v2_4_1_version_surfaces_are_current() -> None:
    assert ENGINE_VERSION_TAG == "v2_4_1"
    assert _read("VERSION").strip() == "v2_4_1"
    assert 'version = "2.4.1"' in _read("pyproject.toml")
    assert 'CONTRACT_VERSION = "v2_4_1"' in _read("src/jammate_agent/core/contract_codegen.py")
...
def test_readme_and_changelog_have_separate_roles() -> None:
    ...
    assert "## v2_4_1" not in readme
    assert "## v2_4_1 — HarmonyOS Generate Contract Sync" in changelog
