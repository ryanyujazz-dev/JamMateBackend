...
def test_v2_4_11_version_surfaces_are_current() -> None:
    assert ENGINE_VERSION_TAG == "v2_4_11"
    assert _read("VERSION").strip() == "v2_4_11"
    assert 'version = "2.4.11"' in _read("pyproject.toml")
    assert 'CONTRACT_VERSION = "v2_4_11"' in _read("src/jammate_agent/core/contract_codegen.py")
